...
public class SiteEditor extends GridPane
{
    ...
    public SiteEditor(UserPreferences userPreferences, PlaylistManager playlistManager)
    {
        ...
        GridPane.setConstraints(getSiteFrequencyTableView(), 0, ++row, 1, 7);
        ...
        createBox.getChildren().addAll(getCreateButton(), getGoToChannelEditorCheckBox());
    ...
    public void setSite(Site site, System system, SystemInformation systemInformation, RadioReferenceDecoder decoder)
    {
        ...
        getCreateButton().setDisable(disable || !supported);
        ...
        if(site != null)
        {
            ...
            getSiteFrequencyTableView().getItems().addAll(siteFrequencies);
            ...
            else
            {
                ...
                if(!siteFrequencies.isEmpty())
                {
                    ...
                    getControlAndAltToggleButton().setDisable(!hasAlternate || p25Hybrid);

                    //Turn-off selection for all toggle buttons
                    ...
                    //Set visibility for configurations toggle group
                    if(p25Hybrid)
                    {
                    ...
                    else if(siteFrequencies.size() == 1)
                    {
                        getSingleToggleButton().setDisable(false);
                        getForEachToggleButton().setDisable(true);
    ...
    private TableView<SiteFrequency> getSiteFrequencyTableView()
    {
        if(mSiteFrequencyTableView == null)
        {
            ...
            mSiteFrequencyTableView.setPrefHeight(200);

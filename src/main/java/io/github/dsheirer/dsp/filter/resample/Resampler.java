...
public class Resampler {
    private static ReusableBufferQueue mReusableBufferQueue = new ReusableBufferQueue("RealResampler");

    public interface SampleBuffers {
        int getInputBufferLength();
        int getOutputBufferLength();
        void produceInput(double[] array, int offset, int length);
        void consumeOutput(double[] array, int offset, int length);
    }
     ...
     */
    public static class BufferManager implements SampleBuffers
    {
        private int mOutputBufferSize;
        private DoubleBuffer mInputBuffer;
        private DoubleBuffer mOutputBuffer;
        ...
        public BufferManager(int inputBufferSize, int outputBufferSize)
        {
            ...
            mInputBuffer = ByteBuffer.allocate(inputBufferSize * 8).asDoubleBuffer();
            mOutputBuffer = ByteBuffer.allocate(inputBufferSize * 8).asDoubleBuffer();
        }

        /**
         * Queues the buffer sample for resampling
         */
        public void load(ReusableDoubleBuffer reusableDoubleBuffer)
        {
            mInputBuffer.put(reusableDoubleBuffer.getSamples());
         ...
         */
        public void produceInput(double[] samples, int offset, int length)
        {
            mInputBuffer.flip();
         ...
         */
        public void consumeOutput(double[] samples, int offset, int length)
        {
            mOutputBuffer.put(samples, offset, length);

            while(mOutputBuffer.position() > mOutputBufferSize)
            {
                ReusableDoubleBuffer outputBuffer = mReusableBufferQueue.getBuffer(mOutputBufferSize);
        ...
        public void setResampledBufferListener(Listener<ReusableDoubleBuffer> resampledBufferListener) {
    ...
    private final double[] ImpD;
    private final double LpScl;
    ...
    public Resampler(boolean highQuality, double minFactor, double maxFactor) {
        ...
        this.Imp = new double[this.Nwing];
        this.ImpD = new double[this.Nwing];
        ...
        this.X = new double[this.XSize + this.Xoff];
        this.Xp = this.Xoff;
    ...
    public boolean process(double factor, SampleBuffers buffers, boolean lastBatch) {
        ...
        double[] Imp = this.Imp;
        double[] ImpD = this.ImpD;
        double LpScl = this.LpScl;
    ...
    /**
     * Process a batch of samples. Convenience method for when the input and output are both doubles.
     *
     * @param factor       factor at which to resample this batch
     ...
     * @return true iff resampling is complete (ie. no input samples consumed and no output samples produced)
     */
    public boolean process(double factor, final DoubleBuffer inputBuffer, boolean lastBatch, final DoubleBuffer outputBuffer) {
        SampleBuffers sampleBuffers = new SampleBuffers() {
            ...
            public void produceInput(double[] array, int offset, int length) {
                inputBuffer.get(array, offset, length);
            }

            public void consumeOutput(double[] array, int offset, int length) {
     ...
     */
    public Result process(double factor, double[] inBuffer, int inBufferOffset, int inBufferLen, boolean lastBatch, double[] outBuffer, int outBufferOffset, int outBufferLen) {
        DoubleBuffer inputBuffer = DoubleBuffer.wrap(inBuffer, inBufferOffset, inBufferLen);
        DoubleBuffer outputBuffer = DoubleBuffer.wrap(outBuffer, outBufferOffset, outBufferLen);
     ...
     * down-conversion;
     */
    private int lrsSrcUp(double X[], double Y[], double factor, int Nx, int Nwing, double LpScl, double Imp[],
                         double ImpD[], boolean Interp) {
    ...
    private int lrsSrcUD(double X[], double Y[], double factor, int Nx, int Nwing, double LpScl, double Imp[],
                         double ImpD[], boolean Interp) {
